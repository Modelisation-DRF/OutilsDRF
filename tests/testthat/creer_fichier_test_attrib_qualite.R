...
liste_arbre <- liste_arbre %>% rename(sdom_bio=sdom) %>%
...
liste_arbre_eq1 <- liste_arbre %>% filter(dhpcm > 23 & dhpcm <= 33) %>% mutate(Equation=1) %>% left_join(qualite0_ass_sdom)
liste_arbre_eq2 <- liste_arbre %>% filter(dhpcm > 33 & dhpcm <= 39) %>% mutate(Equation=2) %>% left_join(qualite0_ass_sdom)
liste_arbre_eq3 <- liste_arbre %>% filter(dhpcm > 39) %>% mutate(Equation=3) %>% left_join(qualite0_ass_sdom)
...
resultatQualiteLourd_tous <- rbind(resultatQualiteLourdEq1, resultatQualiteLourdEq2, resultatQualiteLourdEq3) %>%
  mutate(prop_a = as.numeric(prop_a),
