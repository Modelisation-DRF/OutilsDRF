...
plot <- data.frame(id_pe = c(1, 2, 3), sdom_bio = c("2EST", "3OUEST", "4EST"), tmoy = c(2.1, 0.1, 1.2), ptot = c(828, 700, 1022), sum_st_ha = c(29, 22, 25), coupe = c(0, 0, 1))
...
iter1 <- ex_qualite_evol %>% mutate(iter=1)
iter2 <- ex_qualite_evol %>% mutate(iter=2)
iter3 <- ex_qualite_evol %>% mutate(iter=3)
iter4 <- ex_qualite_evol %>% mutate(iter=4)
iter5 <- ex_qualite_evol %>% mutate(iter=5)
ex_qualite_evol_sto <- bind_rows(iter1,iter2,iter3,iter4,iter5)
usethis::use_data(ex_qualite_evol_sto,
                  internal = FALSE, overwrite = TRUE
)
