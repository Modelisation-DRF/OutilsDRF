...
param_qualite0 <- function(mode_simul='DET', nb_iter=1, seed_value=NULL){
  ...
  # mode_simul='DET'; nb_iter=1; seed_value=NULL;
  # mode_simul='STO'; nb_iter=10; seed_value=NULL;
  ...
  if (mode_simul=='STO') {
    ...
    for(i in 1:length(qualite0_ess)) { # qualite0_ess est un fichier interne
      ...
      bi <- qualite0_param %>% filter(essence==qualite0_ess[i]) %>% dplyr::select(-essence)
      ...
      for(j in 1:3) {
        ...
        bi_qual = as.data.frame(matrix(rockchalk::mvrnorm(n = nb_iter_temp,
                                                             mu = mu,
                                                             Sigma = covb2,
                                                             empirical = T
                                                             ),
        ...
        bi_qual_tr <- bi_qual %>% mutate(iter=row_number()) %>% group_by(iter) %>% pivot_longer(cols=all_of(names(bi_qual)), names_to = 'var', values_to = 'bi')
  ...
  bi_eq1 <- bi_qual_8ess %>% filter(Equation==1) %>% group_by(Equation, iter, essence) %>% pivot_wider(values_from = 'b_i', names_from = 'var_i') %>% ungroup()
  bi_eq2 <- bi_qual_8ess %>% filter(Equation==2) %>% group_by(Equation, iter, essence) %>% pivot_wider(values_from = 'b_i', names_from = 'var_i') %>% ungroup()
  bi_eq3 <- bi_qual_8ess %>% filter(Equation==3) %>% group_by(Equation, iter, essence) %>% pivot_wider(values_from = 'b_i', names_from = 'var_i') %>% ungroup()
