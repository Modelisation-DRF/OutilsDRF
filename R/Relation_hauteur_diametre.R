...
#' DataHt <- relation_h_d(fic_arbres=fic_artemis_sto, mode_simul='STO',
...
relation_h_d<-function (fic_arbres, mode_simul="DET", nb_iter=1, nb_step=1, dt=10, seed_value=NULL, grouping_vars=NULL, reg_eco=FALSE) {
  ...
  if (mode_simul=='STO'){
    if (length(grouping_vars)>0) { stop("grouping_vars ne peut pas etre utilise avec mode_simul=STO")}
...
arbre2 <- fic_arbres[, `:=`(
  ...
  dens = sum(nb_tige * 25, na.rm = TRUE)
    ...
    sdom_bio = fifelse(
      sdom_bio == '1', "1OUEST",
      fifelse(sdom_bio == '2E', "2EST",
      fifelse(sdom_bio == "2O", "2OUEST",
      fifelse(sdom_bio == '3E', '3EST',
      fifelse(sdom_bio == '3O', "3OUEST",
      fifelse(sdom_bio == '4E', "4EST",
      fifelse(sdom_bio == "4O", "4OUEST",
      fifelse(sdom_bio == '5E', '5EST',
      fifelse(sdom_bio == '5O', "5OUEST",
      fifelse(sdom_bio == '6E', '6EST',
...
arbre2[
  , `:=`(
    ...
    eq_pert = fifelse(pert == 'NON', pert_NON * logdhp,
                      fifelse(pert == 'INT', pert_INT * logdhp,
                      fifelse(pert == 'MOY', pert_MOY * logdhp, NA_real_))),

    eq_sdom = fifelse(sdom == '1', sd_1 * logdhp,
                      fifelse(sdom == '2EST', sd_2EST * logdhp,
                      fifelse(sdom == '2OUEST', sd_2OUEST * logdhp,
                      fifelse(sdom == '3EST', sd_3EST * logdhp,
                      fifelse(sdom == '3OUEST', sd_3OUEST * logdhp,
                      fifelse(sdom == '4EST', sd_4EST * logdhp,
                      fifelse(sdom == '4OUEST', sd_4OUEST * logdhp,
                      fifelse(sdom == '5EST', sd_5EST * logdhp,
                      fifelse(sdom == '5OUEST', sd_5OUEST * logdhp,
                      fifelse(sdom == '6EST', sd_6EST * logdhp,
                      fifelse(sdom == '6OUEST', sd_6OUEST * logdhp, NA_real_))))))))))),

    eq_mil = fifelse(milieu == '0', mil_0 * logdhp,
                     fifelse(milieu == '1', mil_1 * logdhp,
                     fifelse(milieu == '2', mil_2 * logdhp,
                     fifelse(milieu == '3', mil_3 * logdhp,
                     fifelse(milieu == '4', mil_4 * logdhp,
                     fifelse(milieu == '5', mil_5 * logdhp,
                     fifelse(milieu == '6', mil_6 * logdhp,
                     fifelse(milieu == '7', mil_7 * logdhp,
                     fifelse(milieu == '8', mil_8 * logdhp,
                     fifelse(milieu == '9', mil_9 * logdhp, NA_real_)))))))))),
  eq_vp = fifelse(vp == 'FC1', vp_FC1 * logdhp,
                  fifelse(vp == 'FE1', vp_FE1 * logdhp,
                  fifelse(vp == 'FE2', vp_FE2 * logdhp,
                  fifelse(vp == 'FE3', vp_FE3 * logdhp,
                  fifelse(vp == 'FE4', vp_FE4 * logdhp,
                  fifelse(vp == 'FE5', vp_FE5 * logdhp,
                  fifelse(vp == 'FE6', vp_FE6 * logdhp,
                  fifelse(vp == 'FO1', vp_FO1 * logdhp,
                  fifelse(vp == 'ME1', vp_ME1 * logdhp,
                  fifelse(vp == 'MF1', vp_MF1 * logdhp,
                  fifelse(vp == 'MJ1', vp_MJ1 * logdhp,
                  fifelse(vp == 'MJ2', vp_MJ2 * logdhp,
                  fifelse(vp == 'MS1', vp_MS1 * logdhp,
                  fifelse(vp == 'XS2' & essence == 'SAB', vp_XS2 * logdhp,
                  fifelse(vp == 'MS2' & essence != 'SAB', vp_MS2 * logdhp,
                  fifelse(vp == 'MS4', vp_MS4 * logdhp,
                  fifelse(vp == 'MS6', vp_MS6 * logdhp,
                  fifelse(vp == 'RB1', vp_RB1 * logdhp,
                  fifelse(vp == 'RB5', vp_RB5 * logdhp,
                  fifelse(vp == 'RC3', vp_RC3 * logdhp,
                  fifelse(vp == 'RE1', vp_RE1 * logdhp,
                  fifelse(vp == 'RE2', vp_RE2 * logdhp,
                  fifelse(vp == 'RE3', vp_RE3 * logdhp,
                  fifelse(vp == 'RE4', vp_RE4 * logdhp,
                  fifelse(vp == 'RP1', vp_RP1 * logdhp,
                  fifelse(vp == 'RS1', vp_RS1 * logdhp,
                  fifelse(vp == 'RS2', vp_RS2 * logdhp,
                  fifelse(vp == 'RS3', vp_RS3 * logdhp,
                  fifelse(vp == 'RS4', vp_RS4 * logdhp,
                  fifelse(vp == 'RS5', vp_RS5 * logdhp,
                  fifelse(vp == 'RT1', vp_RT1 * logdhp, NA_real_)))))))))))))))))))))))))))))))
)
]
# Calcul de hauteur_pred
arbre2[, hauteur_pred := fifelse(!is.na(essence) & dhpcm > 9,
