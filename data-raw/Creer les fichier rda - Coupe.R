...
verif <- param %>%
  filter(estimate == 0) %>%
  group_by(num_trt) %>%
  summarise(nb = n()) # il y a 2 trt sans 0, les 12 et 14
...
coupe_ess2 <- coupe_ess %>%
  group_by(SpeciesName) %>%
  pivot_longer(cols = CA:CPI_RL_CIMOTF, names_to = "code_trt", values_to = "essence_coupe") %>%
  rename(essence = SpeciesName)
# ajouter le numérode du trt
list_trt <- param %>%
  select(num_trt, code_trt) %>%
  unique()
...
effet_presents <- param_ess2 %>%
  ungroup() %>%
  select(-code_trt, -num_trt, -essence) %>%
  names()
