test_that("La fonction param_coupe génère les bons paramètres en mode DET pour trt=0", {
  ...
  obtenu_sans_ess <- obtenu %>%
    select(b0, b4_s, b5_s, b6_s) %>%
    unique()
  ...
  attendu <- attendu %>%
    select(essence, estimate, effect) %>%
    group_by(essence) %>%
  ...
  # paramètres qui ne dépendent pas de l'essence
  attendu_sans_ess <- attendu %>%
    select(essence, b0, b4, b5, b6) %>%
    filter(is.na(essence)) %>%
    slice(1) %>%
    ungroup() %>%
    select(-essence)
