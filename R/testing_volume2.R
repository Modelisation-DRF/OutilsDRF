...
get_diam <- function(fic) {
  ...
  process_species <- function(ess) {
    data_ess <- data_filtre[essence == ess]
    data_ess_na <- data.table()  # Initialize here
    data_ess_non_na <- data.table()  # Initialize here

    # Only process groups if not PIB
    if (nrow(data_ess) > 0 && ess != 'PIB') {
      group_mapping <- c(
        'veg_pot' = 'defil_group_vp',
        'sdom_bio' = 'defil_group_sd',
        'cl_drai' = 'defil_group_dr'
      )

      name_mapping <- c(
        'veg_pot' = 'group.veg',
        'sdom_bio' = 'group.sDomBio',
        'cl_drai' = 'group.drainage'
      )

      for (group_col in names(group_mapping)) {
        group_var <- group_mapping[group_col]
        group_dt <- get(group_var)
        group_name <- name_mapping[group_col]

        if (!is.null(group_dt[[ess]]) && any(!is.na(group_dt[[ess]]))) {
          group_temp <- copy(group_dt)
          group_temp[, (group_name) := get(ess)]
          data_ess <- data_ess[group_temp[, .SD, .SDcols = c(group_col, group_name)],
                               on = group_col, nomatch = 0]
        }
      }
    ...
    if (nrow(data_ess_na) > 0 || ess == 'PIB') {
      data_ess_na <- data_ess  # Use full dataset for tree-only model
    ...
    return(combined_data[, .SD, .SDcols = cols_to_keep])
  }

  result <- lapply(defil_liste_ess, process_species)
  return(rbindlist(result[!sapply(result, is.null)], fill=TRUE))
}
...
calcul_vol_bille <- function(fichier_billes) {
  ...
  if (nrow(data_filtre) > 0) {
    ...
    print("check1")
    timing <- system.time({
    new_data <- expanded_data[, `:=`(DIAM_PREDICT = sqrt(get_diam(.SD)$pred_mm2_corr) / ratio_mm_metre),
                              by = .(id_pe, no_arbre)]
    })
    print(timing)
    print("check1.5")
