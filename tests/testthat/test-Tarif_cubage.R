test_that("cubage() avec mode déterministe et fichier param fourni estime les bons volumes", {
  ...
  verif_obtenu <- DataVol %>% rename(vol_obtenu = vol_dm3) %>% select(id_pe, no_arbre, essence, vol_obtenu)
  verif_attendu <- data_arbre_attendu %>% rename(vol_attendu = vol_dm3) %>% select(id_pe, no_arbre, essence, vol_attendu)
...
test_that("cubage() avec mode stochastique estime les bons volumes", {
  ...
  verif_obtenu <- DataVol %>% rename(vol_obtenu = vol_dm3) %>% select(id_pe, no_arbre, essence, iter, step, vol_obtenu)
  verif_attendu <- data_arbre_attendu_sto %>% rename(vol_attendu = vol_dm3) %>% select(id_pe, no_arbre, essence, iter, step, vol_attendu)
...
test_that("cubage() avec un fichier de samare estime les bons volumes", {
  ...
  verif_obtenu <- data_simul_samare_obtenu %>% rename(vol_obtenu = vol_dm3) %>% select(id_pe, no_arbre, essence, iter, step, vol_obtenu)
  verif_attendu <- data_simul_samare_attendu_2 %>% rename(vol_attendu = vol_dm3) %>% select(id_pe, no_arbre, essence, iter, step, vol_attendu)
  compare <- inner_join(verif_attendu, verif_obtenu, by=c("id_pe", "no_arbre", "essence", "step", "iter")) %>% mutate(diff = round(vol_attendu-vol_obtenu,2))
